...
public class Menu : MonoBehaviour
{
    ...
    [Header("--- NAVIGATION ---")]
    public GameObject[] tuttiIBottoniMenu; // Trascina qui i 15 bottoni singoli
    ...
    // ==========================================
    // VALIDAZIONE INPUT (Mentre l'utente scrive)
    // ==========================================
    ...
    private void SpegniSoloAR()
    {
        ...
        if (prezzo) prezzo.text = ""; if (testoTitoloPiatto) testoTitoloPiatto.text = "";
        if (testoIngredienti) testoIngredienti.text = ""; if (testoAllergie) testoAllergie.text = "";
    ...
    public void ChiudiCarrello() { if (pannelloCarrello) pannelloCarrello.SetActive(false); }
    ...
    {
        ResetAllergeniEBottoni();
        if (toggleGlutine && toggleGlutine.isOn) DisattivaFiltro(new Button[] { btnPizza, btnPane, btnCroassaint, btnHamburger, btnCheeseburger, btnGianburger, btnTorta, btnFocaccia });
        if (toggleLattosio && toggleLattosio.isOn) DisattivaFiltro(new Button[] { btnPizza, btnCappuccino, btnCroassaint, btnCheeseburger, btnGianburger, btnTorta });
        if (toggleVegano && toggleVegano.isOn) DisattivaFiltro(new Button[] { btnPizza, btnCappuccino, btnPollo, btnSushi, btnHamburger, btnCheeseburger, btnGianburger, btnTorta, btnSandwich });
    }

    private void DisattivaFiltro(Button[] lista) { foreach (Button b in lista) if (b) b.interactable = false; }
    private void ResetAllergeniEBottoni() { Button[] tutti = { btnPizza, btnCappuccino, btnPane, btnCroassaint, btnPollo, btnPatatine, btnSushi, btnKashipan, btnHamburger, btnCheeseburger, btnGianburger, btnTorta, btnSandwich, btnTofu, btnFocaccia }; foreach (Button b in tutti) if (b != null) b.interactable = true; }

    // ==========================================
    // COROUTINES / FEEDBACK
    // ==========================================
    IEnumerator NascondiIstruzioni(float s) { yield return new WaitForSeconds(s); if (testoIstruzioniAR) testoIstruzioniAR.SetActive(false); if (sfondoNeroIstruzioniAR) sfondoNeroIstruzioniAR.SetActive(false); }
    IEnumerator NascondiFeedbackAggiunta(float s) { yield return new WaitForSeconds(s); if (testoPiattoAggiunto) testoPiattoAggiunto.gameObject.SetActive(false); }
    IEnumerator NascondiOrdineInviato(float s) { yield return new WaitForSeconds(s); if (testoOrdineInviato) testoOrdineInviato.gameObject.SetActive(false); ChiudiCarrello(); }
    public void VibraTargetTrovato() { Handheld.Vibrate(); }

    // ==========================================
    // APERTURA PIATTI
    // ==========================================
    public void openPizza() { prezzoAttuale = 8.50f; PreparazionePiatto(); if (pizza) pizza.SetActive(true); prezzo.text = "8.50 EUR"; testoTitoloPiatto.text = "PIZZA MARGHERITA"; testoIngredienti.text = "Pomodoro, Mozzarella, Basilico"; testoAllergie.text = "⚠️ Glutine, Lattosio"; }
    public void openCappuccino() { prezzoAttuale = 1.50f; PreparazionePiatto(); if (cappuccino) cappuccino.SetActive(true); prezzo.text = "1.50 EUR"; testoTitoloPiatto.text = "CAPPUCCINO"; testoIngredienti.text = "Caffè, Latte"; testoAllergie.text = "⚠️ Lattosio"; }
    public void openPane() { prezzoAttuale = 2.00f; PreparazionePiatto(); if (Pane) Pane.SetActive(true); prezzo.text = "2.00 EUR"; testoTitoloPiatto.text = "PANE CASERECCIO"; testoIngredienti.text = "Farina, Acqua, Lievito"; testoAllergie.text = "⚠️ Glutine"; }
    public void openCroassaint() { prezzoAttuale = 1.50f; PreparazionePiatto(); if (Croassaint) Croassaint.SetActive(true); prezzo.text = "1.50 EUR"; testoTitoloPiatto.text = "CROISSAINT AL BURRO"; testoIngredienti.text = "Farina, Burro, Uova"; testoAllergie.text = "⚠️ Glutine, Lattosio, Uova"; }
    public void openPollo() { prezzoAttuale = 12.00f; PreparazionePiatto(); if (Pollo) Pollo.SetActive(true); prezzo.text = "12.00 EUR"; testoTitoloPiatto.text = "POLLO ARROSTO"; testoIngredienti.text = "Pollo ruspante, Rosmarino"; testoAllergie.text = "✅ Senza allergeni"; }
    public void openPatatineecoca() { prezzoAttuale = 6.00f; PreparazionePiatto(); if (Patatineecoca) Patatineecoca.SetActive(true); prezzo.text = "6.00 EUR"; testoTitoloPiatto.text = "PATATINE E COCA COLA"; testoIngredienti.text = "Patate, Bibita"; testoAllergie.text = "✅ Senza allergeni"; }
    public void openSushi() { prezzoAttuale = 15.00f; PreparazionePiatto(); if (Sushi) Sushi.SetActive(true); prezzo.text = "15.00 EUR"; testoTitoloPiatto.text = "SUSHI MIX"; testoIngredienti.text = "Riso, Salmone, Soia"; testoAllergie.text = "⚠️ Pesce, Soia"; }
    public void openKashipan() { prezzoAttuale = 3.50f; PreparazionePiatto(); if (Kashipan) Kashipan.SetActive(true); prezzo.text = "3.50 EUR"; testoTitoloPiatto.text = "KASHIPAN"; testoIngredienti.text = "Pane dolce, Burro"; testoAllergie.text = "⚠️ Glutine, Lattosio"; }
    public void openHambuger() { prezzoAttuale = 9.00f; PreparazionePiatto(); if (Hambuger) Hambuger.SetActive(true); prezzo.text = "9.00 EUR"; testoTitoloPiatto.text = "HAMBURGER"; testoIngredienti.text = "Pane, Manzo, Maionese"; testoAllergie.text = "⚠️ Glutine, Uova"; }
    public void openCheeseburger() { prezzoAttuale = 10.00f; PreparazionePiatto(); if (Cheeseburger) Cheeseburger.SetActive(true); prezzo.text = "10.00 EUR"; testoTitoloPiatto.text = "CHEESEBURGER"; testoIngredienti.text = "Pane, Manzo, Cheddar"; testoAllergie.text = "⚠️ Glutine, Lattosio"; }
    public void openGianburger() { prezzoAttuale = 13.50f; PreparazionePiatto(); if (Gianburger) Gianburger.SetActive(true); prezzo.text = "13.50 EUR"; testoTitoloPiatto.text = "GIANT BURGER"; testoIngredienti.text = "Doppio Manzo, Bacon, Cheddar"; testoAllergie.text = "⚠️ Glutine, Lattosio"; }
    public void openTorta() { prezzoAttuale = 5.00f; PreparazionePiatto(); if (Torta) Torta.SetActive(true); prezzo.text = "5.00 EUR"; testoTitoloPiatto.text = "TORTA"; testoIngredienti.text = "Pan di Spagna, Crema, Fragole"; testoAllergie.text = "⚠️ Glutine, Lattosio, Uova"; }
    public void openSandwich() { prezzoAttuale = 6.50f; PreparazionePiatto(); if (Sandwich) Sandwich.SetActive(true); prezzo.text = "6.50 EUR"; testoTitoloPiatto.text = "CLUB SANDWICH"; testoIngredienti.text = "Pane, Tacchino, Bacon, Uova"; testoAllergie.text = "⚠️ Glutine, Uova"; }
    public void openTofu() { prezzoAttuale = 7.00f; PreparazionePiatto(); if (Tofu) Tofu.SetActive(true); prezzo.text = "7.00 EUR"; testoTitoloPiatto.text = "TOFU"; testoIngredienti.text = "Soia, Salsa Soia"; testoAllergie.text = "⚠️ Soia"; }
    public void openFocaccia() { prezzoAttuale = 4.00f; PreparazionePiatto(); if (Focaccia) Focaccia.SetActive(true); prezzo.text = "4.00 EUR"; testoTitoloPiatto.text = "FOCACCIA"; testoIngredienti.text = "Farina, Olio, Sale"; testoAllergie.text = "⚠️ Glutine"; }
}